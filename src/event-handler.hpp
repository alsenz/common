...
namespace caf {
    ...
    namespace detail {
        ...
        struct behaviors_to_throw_away_handler<typed_mpi < detail::type_list < Is...>, output_tuple<void>>> {
            ...
            auto operator()(T *thiz) {
                return [thiz](Is... args) -> void {
                    std::stringstream ss;
                    ss << "Handling throwaway event. This type: " << typeid(T).name() << ", args tuple: "
                       << typeid(std::tuple<Is...>).name();
                    as::common::logger::trace_a(thiz, ss.str(), "event-handler", __FUNCTION__, __LINE__, __FILE__);
    ...
    // Add the reducer method in as a mixin for now -- hopefully make fan_out_request more generic in the future.
    template<class Subtype, typename... Events>
    using event_handler_base = mixin::reducer<detail::event_typed_actor<Events..., throw_away_event_t<Events>...>::base, Subtype>;

    template<typename... Events>
    class event_handler : public event_handler_base<event_handler<Events...>, Events...>, public as::virtual_handler_base<Events...> {
